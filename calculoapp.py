...
def rendimento_desejado(codigo, valor_para_investir):
    if codigo in df['CÓDIGO'].values:
        ...
        st.write("Código não encontrado no DataFrame.")
...
# Menu suspenso para selecionar o código
codigo_selecionado = st.sidebar.selectbox('Selecione o Código', [''] + list(df['CÓDIGO'].unique()))

# Verificar se um código foi selecionado
if codigo_selecionado != '':
    # Definir o valor do código selecionado
    codigo = codigo_selecionado

    # Extrair o valor do preço atual para o código selecionado
    valor_preco = df.loc[df['CÓDIGO'] == codigo, 'COTA'].values[0]

    # Extrair o valor do dividendo para o código selecionado
    valor_rendimento = df.loc[df['CÓDIGO'] == codigo, 'DIVIDENDO'].values[0]

    # Mostrar o valor do preço atual e do dividendo para o código selecionado
    st.write(f"Preço atual para {codigo}: R$ {valor_preco:.2f}")
    st.write(f"Dividendo para {codigo}: R$ {valor_rendimento:.2f}")
    ...
    if st.sidebar.button("Consultar"):
        rendimento_desejado(codigo, valor_para_investir)