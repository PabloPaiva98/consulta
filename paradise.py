...
def adicionar_a_carteira(codigo, valor_para_investir, carteira):
    if codigo in df['CÓDIGO'].values:
        valor_preco = df.loc[df['CÓDIGO'] == codigo, 'COTA'].values[0]
        valor_rendimento = df.loc[df['CÓDIGO'] == codigo, 'DIVIDENDO'].values[0]
...
codigo_para_carteira = st.sidebar.selectbox('Selecione o Código para Adicionar à Carteira', [''] + list(df_filtrado['CÓDIGO'].unique()))
...
if st.sidebar.button("Adicionar à Carteira"):
    adicionar_a_carteira(codigo_para_carteira, valor_para_investir_carteira, carteira)
    st.success(f"{codigo_para_carteira} adicionado à carteira com R$ {valor_para_investir_carteira:.2f} investidos.")

# Mostrar carteira
st.subheader("Carteira")
st.write(pd.DataFrame(carteira))